...
class SendtochannelCommand extends AdminCommand {
    ...
    public function execute() {
        ...
        $yes_no_add_keyboard = new Keyboard(
            [
                'keyboard'          => [['Yes', 'No'], ['Add End Picture']],
                'resize_keyboard'   => true,
                'one_time_keyboard' => true,
                'selective'         => true,
            ]
        ...
        switch ($state) {
            ...
            case 4:
                ...
                if ((!$text_yes_or_no || $notes['last_message_id'] === $message->getMessageId()) && $text !== 'Add End Picture') {
                    $notes['state'] = 4;
                    $this->conversation->update();
                    ...
                    if ($notes['message_type'] !== 'command') {
                        ...
                        if ($notes['end_picture_link']) {
                            $notes['message']['text'] .= PHP_EOL . "<a href='{$notes['end_picture_link']}'>&#160;</a>";
                        }

                        $this->sendBack(
                            new Message($notes['message'], $this->telegram->getBotUsername()),
                            array_merge($data, ['parse_mode' => 'html'])
                        );

                        $data['reply_markup'] = (isset($notes['end_picture_link'])) ? $yes_no_keyboard : $yes_no_add_keyboard;

                        $result = $this->replyToChat('Would you like to post it?', $data);
                    }
                    break;
                }

                $notes['post_message'] = $text;
                $notes['last_message_id'] = $message->getMessageId();
            // no break
            case 5:

                if ($text === "Add End Picture" && $notes['last_message_id'] === $message->getMessageId()) {
                    $notes['state'] = 5;
                    $this->conversation->update();
                ...
                if (!$text_yes_or_no && !isset($notes['end_picture_link'])) {
                    $notes['end_picture_link'] = $text;
                    goto state_4;
                }
            ...
            case 6:
                ...
                if (strtolower($notes['post_message']) === 'yes') {
